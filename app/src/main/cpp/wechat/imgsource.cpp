...
// Added for crop function
        ImgSource::ImgSource(unsigned char *pixels, int width, int height, int left_, int top_,
            ...
            if ((left_ + cropWidth) > dataWidth || (top_ + cropHeight) > dataHeight || top_ < 0 ||
                left_ < 0) {
                err_handler =
                        zxing::IllegalArgumentErrorHandler(
        ...
        Ref<ImgSource>
        ImgSource::create(unsigned char *pixels, int width, int height, int left, int top,
                          int cropWidth, int cropHeight,
                          zxing::ErrorHandler &err_handler) {
            return Ref<ImgSource>(
                    new ImgSource(pixels, width, height, left, top, cropWidth, cropHeight,
                                  err_handler));
        }
        ...
        ArrayRef<char> ImgSource::getRow(int y, zxing::ArrayRef<char> row,
            ...
            if (y < 0 || y >= getHeight()) {
                err_handler = zxing::IllegalArgumentErrorHandler(
...
/** This is a more efficient implementation. */
        ...
        Ref<LuminanceSource> ImgSource::crop(int left_, int top_, int width, int height,
            ...
            return ImgSource::create(rgbs, dataWidth, dataHeight, left + left_, top + top_, width,
                                     height, err_handler);
        }
        ...
        Ref<LuminanceSource> ImgSource::rotateCounterClockwise(ErrorHandler &err_handler) const {
            ...
            return ImgSource::create(rgbs, dataWidth, dataHeight, top, left, height, width,
                                     err_handler);
        }
