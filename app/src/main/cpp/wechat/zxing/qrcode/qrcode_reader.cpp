...
namespace zxing {
    namespace qrcode {
                ...
                for (int i = 0; i < possiblePatternCount; i++) {
                    ...
                    for (int j = 0; j < possibleAlignmentCount; j++) {
                        ...
                        Ref<DetectorResult> detectorResult =
                                detector->getResultViaAlignment(i, j, detectedDimension_,
                                                                err_handler);
                        if (err_handler.ErrCode()) {
                            ...
                            if ((patternInfo->getPossibleFix() > 0.9 &&
                                 decoder_.getPossibleFix() < 0.1)) {
                        ...
                        Ref<Result> result(
                                new Result(decoderResult->getText(), decoderResult->getRawBytes(),
                                           points,
                    ...
                    for (int j = 0; j < possibleAlignmentCount; j++) {
                        ...
                        if (needTryVariousDeimensions[j]) {
                            vector<int> possibleDimensions = getPossibleDimentions(
                                    detectedDimension_);
                            for (size_t k = 1; k < possibleDimensions.size(); k++) {
                                ...
                                Ref<DetectorResult> detectorResult =
                                        detector->getResultViaAlignment(i, j, dimension,
                                                                        err_handler);
        ...
        int
        QRCodeReader::smooth(unsigned int *integral, Ref<BitMatrix> input, Ref<BitMatrix> output,
                             int window) {
