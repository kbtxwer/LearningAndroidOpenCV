...
// Orders by furthes from average
            ...
            public:
                explicit CenterComparator(float averageModuleSize) : averageModuleSize_(
                        averageModuleSize) {}
            ...
            public:
                bool operator()(Ref<FinderPattern> a, Ref<FinderPattern> b) {
                    if (a->getCount() != b->getCount()) {
                        ...
                        int aErr = 0, bErr = 0;
                        if (a->getHorizontalCheckState() !=
                            FinderPattern::HORIZONTAL_STATE_NORMAL)
                            aErr++;
                        if (a->getVerticalCheckState() != FinderPattern::VERTICAL_STATE_NORMAL)
                            aErr++;
                        if (b->getHorizontalCheckState() !=
                            FinderPattern::HORIZONTAL_STATE_NORMAL)
                            bErr++;
                        if (b->getVerticalCheckState() != FinderPattern::VERTICAL_STATE_NORMAL)
                            bErr++;
        ...
        std::vector<Ref<FinderPatternInfo>> FinderPatternFinder::find(DecodeHints const &hints,
                ...
                for (size_t i = iSkip - 1; i < maxI; i += iSkip) {
                    ...
                    for (size_t rj = matrix.getRowFirstIsWhite(i) ? 1 : 0;
                        ...
                        if (block_->GetUnicomBlockIndex(i, irow_offsets[rj]) ==
                            ...
                            const int iWhiteCir = block_->GetUnicomBlockSize(i,
                                                                             irow_offsets[rj + 1]);
                            const int iBlackPnt = block_->GetUnicomBlockSize(i,
                                                                             irow_offsets[rj + 2]);
            ...
            vector<Ref<FinderPatternInfo>> patternInfos = getPatternInfosFileMode(hints,
                                                                                  err_handler);
            ...
            for (size_t index = 0; index < possibleCenters_.size(); index++) {
                ...
                if (center->aboutEquals(estimatedModuleSize, centerI, centerJ)) {
                    ...
                    possibleCenters_[index]->setHorizontalCheckState(
                            horizontalState == FinderPatternFinder::NORMAL
                            ? center->getHorizontalCheckState()
                            : horizontalState);
                    possibleCenters_[index]->setVerticalCheckState(
                            verticalState == FinderPatternFinder::NORMAL
                            ? center->getVerticalCheckState()
                            : verticalState);
                    return false;
                }
            }
        ...
        bool
        FinderPatternFinder::isEqualResult(Ref<FinderPatternInfo> src, Ref<FinderPatternInfo> dst) {
            if (src == NULL) {
                return false;
            }
            ...
            return topLeft->aboutEquals(1.0, dst->getTopLeft()->getY(),
                                        dst->getTopLeft()->getX()) &&
                   bottomLeft->aboutEquals(1.0, dst->getBottomLeft()->getY(),
                                           dst->getBottomLeft()->getX()) &&
                   topRight->aboutEquals(1.0, dst->getTopRight()->getY(),
                                         dst->getTopRight()->getX());
        }

        bool
        FinderPatternFinder::IsPossibleFindPatterInfo(Ref<FinderPattern> a, Ref<FinderPattern> b,
                                                      Ref<FinderPattern> c) {
            // check variance
            ...
            float val =
                    sqrt((aMs - avg) * (aMs - avg) + (bMs - avg) * (bMs - avg) +
                         (cMs - avg) * (cMs - avg));
        ...
        vector<Ref<FinderPatternInfo>> FinderPatternFinder::getPatternInfosFileMode(
            ...
            if (startSize == 3) {
                PushToResult(possibleCenters_[0], possibleCenters_[1], possibleCenters_[2],
                             patternInfos);
                return patternInfos;
            }
            ...
            if (FP_IS_SELECT_BEST) {
                ...
                if (err_handler.ErrCode() == 0)
                    PushToResult(finderPatterns[0], finderPatterns[1], finderPatterns[2],
                                 patternInfos);
            }
            ...
            for (size_t i = 0; i < possibleCenters_.size(); i++) {
                if (possibleCenters_[i]->getEstimatedModuleSize() >= FP_MS_MIN &&
                    ...
                    if (hints.getUseNNDetector() &&
                        standardCenters.size() >= size_t(FP_INPUT_CNN_MAX_NUM))
                        break;
                }
            }
                ...
                for (size_t x = 0; x < standardCenters.size(); x++) {
                    for (size_t y = x + 1; y < standardCenters.size(); y++) {
                        for (size_t z = y + 1; z < standardCenters.size(); z++) {
                            ...
                            if (check_result) {
                                PushToResult(standardCenters[x], standardCenters[y],
                                             standardCenters[z],
                                             patternInfos);
                            }
                        }
            ...
            for (size_t i = 0; i < clusters_out.size(); i++) {
                ...
                for (size_t x = 0;
                    ...
                    for (size_t y = x + 1;
                         ...
                         y++) {
                        for (size_t z = y + 1;
                             z < clusters_out[i].samples.size() &&
                             cluster_select <= FPS_CLUSTER_MAX &&
                             patternInfos.size() <= size_t(FPS_RESULT_MAX);
                             z++) {
                            bool check_result = IsPossibleFindPatterInfo(
                                    clusterPatterns[x], clusterPatterns[y], clusterPatterns[z]);
                            if (check_result) {
                                PushToResult(clusterPatterns[x], clusterPatterns[y],
                                             clusterPatterns[z],
                                             patternInfos);
                                cluster_select++;
                            }
...
// position, figures the location of the center of this run.
        float FinderPatternFinder::centerFromEnd(int *stateCount, int end) {
            ...
            return (float) (end - stateCount[4]) -
                   (stateCount[3] + stateCount[2] + stateCount[1]) / 2.0f;
        }
...
// used by finder patterns to be considered a match
        ...
        int FinderPatternFinder::getMinModuleSize() {
            int minModuleSize =
                    (3 * min(image_->getWidth(), image_->getHeight())) / (4 * MAX_MODULES);
 ...
 */
        bool FinderPatternFinder::crossCheckDiagonal(int startI, int centerJ, int maxCount,
            ...
            while ((startI - i >= 0) && (centerJ - i >= 0) &&
                   !matrix.get(centerJ - i, startI - i) &&
        ...
        int FinderPatternFinder::getStateCountTotal(int *stateCount,
                                                    const CrossCheckState &check_state) {
...
// alignment pattern.
        ...
        int FinderPatternFinder::addHorizontalCheckedResult(size_t startJ, size_t centerI,
                                                            float centerJ) {
 ...
 */
        bool FinderPatternFinder::handlePossibleCenter(int *stateCount, size_t i, size_t j) {
            ...
            float tolerateModuleSize =
                    estimatedHorizontalModuleSize > 4.0 ? estimatedHorizontalModuleSize / 2.0f
                                                        : 1.0f;
            ...
            for (int k = 0; k < CENTER_CHECK_TIME; k++) {
                ...
                if (!isnan(centerI) && centerI >= 0.0) {
                    ...
                    float moduleSizeDiff = abs(
                            estimatedHorizontalModuleSize - estimatedVerticalModuleSize);
                    ...
                    tolerateModuleSize =
                            estimatedVerticalModuleSize > 4.0 ? estimatedVerticalModuleSize / 2.0f
                                                              : 1.0f;
                    ...
                    for (int l = 0; l < CENTER_CHECK_TIME; l++) {
                        ...
                        if (!isnan(cJ) && cJ >= 0.0) {
                            ...
                            float ccj =
                                    crossCheckHorizontal((size_t) cJ, (size_t) possibleCenterI,
                                                         stateCount[2],
                                                         stateCountTotal,
                                                         reEstimatedHorizontalModuleSize);
...
// size differs from the average among those patterns the least. //
        ...
        FinderPatternFinder::selectBestPatterns(ErrorHandler &err_handler) {
            size_t startSize = possibleCenters_.size();
            ...
            if ((possibleCenters_[2]->getCount() - possibleCenters_[3]->getCount()) > 1 &&
                ...
                for (int i = 0; i < 4; i++) {
                    float diff = abs(
                            possibleCenters_[i]->getEstimatedModuleSize() - everageModuleSize);
                ...
                return result;
            } else if (possibleCenters_[1]->getCount() > 1 &&
                       possibleCenters_[2]->getCount() == 1) {
            ...
            if (startSize > 3) {
                ...
                for (size_t i = 0;
                     i < possibleCenters_.size() && possibleCenters_.size() > 3; i++) {
        ...
        vector<Ref<FinderPattern>>
        FinderPatternFinder::selectFileBestPatterns(ErrorHandler &err_handler) {
            size_t startSize = possibleCenters_.size();
            ...
            for (size_t i = 0; i < possibleCenters_.size() - 2; ++i) {
                ...
                for (size_t j = 0; j < 3; ++j) {
                    ...
                    fModuleSizeDiff += fabs(possibleCenters_[i + j]->getEstimatedModuleSize() -
                                            possibleCenters_[i + ((j + 1) %
                                                                  3)]->getEstimatedModuleSize());
                }
...
// AB<AC and BC<AC and the angle between BC and BA is less than 180 degrees.
        ...
        bool FinderPatternFinder::checkIsoscelesRightTriangle(Ref<FinderPattern> centerA,
                                                              ...
                                                              Ref<FinderPattern> centerC,
                                                              float &longSide) {
            float shortSide1, shortSide2;
            FinderPatternInfo::calculateSides(centerA, centerB, centerC, longSide, shortSide1,
                                              shortSide2);
            ...
            auto maxModuleSize =
                    maxAmongThree(centerA->getEstimatedModuleSize(),
                                  centerB->getEstimatedModuleSize(),
                                  centerC->getEstimatedModuleSize());
            // if edge length smaller than 14 * module size
            if (minSide <= maxModuleSize * 14) return false;

            float CosLong =
                    (shortSide1 + shortSide2 - longSide) / (2 * shortSideSqrt1 * shortSideSqrt2);
            float CosShort1 =
                    (longSide + shortSide1 - shortSide2) / (2 * longSideSqrt * shortSideSqrt1);
            float CosShort2 =
                    (longSide + shortSide2 - shortSide1) / (2 * longSideSqrt * shortSideSqrt2);
...
// return distance between two points
        ...
        vector<Ref<FinderPattern>> &
        FinderPatternFinder::getPossibleCenters() { return possibleCenters_; }
